...
public class InterceptingClassTransformer implements ClassFileTransformer {
	...
	public byte[] transform(ClassLoader loader, String className, Class<?> classBeingRedefined,
		...
		if (className.contains("com/")) {
			System.out.println("classes are available in another project " + className);
		}
		if (className.contains("com/")) {
			try {
				...
				for (CtMethod method : methods) {
					...
					method.insertAfter(buffer.toString());
					method.insertBefore("System.out.println(\"Logging using Agent Before\");");
					method.insertAfter("System.out.println(\"Logging using Agent After\");");
